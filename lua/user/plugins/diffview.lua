...
keymap("n", "<leader>gm", ":DiffviewOpen<CR>", optsdesc("git merge"))


-- local actions = diffview.actions -- not the same as below!!
local actions = require("diffview.actions")

-- Settings (https://github.com/sindrets/diffview.nvim#readme)
diffview.setup({
	...
	keymaps = {
		...
		view = {
			...
			["<C-l>"]       = actions.select_next_entry, -- Open the diff for the next file
			["<C-h>"]       = actions.select_prev_entry, -- Open the diff for the previous file
		...
		diff3 = {
			...
			{ { "n", "x" }, "<leader>gch", actions.diffget("ours") }, -- Obtain the diff hunk from the OURS version of the file
			{ { "n", "x" }, "<leader>gcl", actions.diffget("theirs") }, -- Obtain the diff hunk from the THEIRS version of the file
		},
		diff4 = {
		...
		file_panel = {
			...
			-- ["l"]				= actions.focus_files,
			["<leader>gu"]    = actions.toggle_files,
			-- ["g<C-x>"]	    = actions.cycle_layout,
			["<leader>gp"]    = actions.prev_conflict,
			["<leader>gn"]    = actions.next_conflict,
		},
		file_history_panel = {
